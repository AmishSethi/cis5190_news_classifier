...
.
├── README.md                  # this file
├── REPORT.md                  # detailed write-up (5-page report material)
├── requirements.txt           # pinned dependencies
├── demo.ipynb                 # live-demo notebook (matches the report)
├── data/                      # canonical training and evaluation splits
│   ├── all_clean.csv          # 52,530 deduplicated scraped headlines
│   ├── course_only.csv        # 3,801 course-released starter headlines
│   ├── co_val_temp.csv        # 7,532 — final training file
│   ├── val_frozen.csv         # 1,967 — random val split, frozen
│   ├── temporal_val_frozen.csv # 1,764 — most-recent 5%, leaderboard proxy
│   ├── train.csv              # 48,790 — full curated train set
│   └── val.csv                # initial (pre-freeze) random val
├── scripts/                   # all training / evaluation / packaging code
├── submission/                # final submission (model.py + preprocess.py)
└── project-resources/
    └── Newsheadlines/         # course-provided eval harness + starter CSV
```

`submission/model.pt` (the 600 MB checkpoint) is **not** stored in git; it is regenerated by running the training scripts (see *Reproducing the final model* below) or you can download an identical artifact from the public dataset card.
...
- **Live demo notebook:** [open `demo.ipynb` in Colab](https://colab.research.google.com/github/alliemi/cis5190-finalproject/blob/main/demo.ipynb).
...
The committed `data/` directory already has all five splits ready to use, so this step is optional.
